...
function initializeChatBot() {
  ...
  const botId = "xxxxxxxx"
  const botAliasId = "xxxxxxxxx"
  let sessionState = {};
  ...
  const processResponse = (val) => {
    let output = "";
    let messageData = val.messages[0];
    let message = messageData.content;
    let type = messageData.contentType;
    if (val && val.messages.length) {
      switch (type) {
  ...
  const send = (text = "") => {
    ...
    lexruntime.recognizeText(params, function (err, data) {
      ...
      if (data) {
        // Capture the sessionState for the next cycle
